...
class VOCDataset(BaseDataSet):
    ...
    def _load_data(self, index):
        ...
        image = np.asarray(Image.open(image_path), dtype=np.float32)  # <class 'numpy.ndarray'>
        label = np.asarray(Image.open(label_path), dtype=np.int32)  # label是语义分割后的png图像
        image_id = self.files[index].split("/")[-1].split(".")[0]  # id是图像名称
