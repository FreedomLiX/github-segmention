...
def main():
    ...
    assert dataset_type in ['VOC', 'COCO', 'CityScapes', 'ADE20K', 'TianChi', 'Meter']  # insert item 'TianChi' 'Meter'
    ...
    # load
    model.load_state_dict(checkpoint, False)  # insert parameter 'False' in order to ignore the error RuntimeError: Error(s) in loading state_dict for DataParallel: Missing key(s) in state_dict: "module.initial.0.0.weight", "module.initial.0.1.weight", ...
    model.to(device)
    model.eval()
